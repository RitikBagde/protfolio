...
const DEFAULT_STACK_DEPTH = 4;
const DEFAULT_AUTOPLAY_INTERVAL = 4500;
const STEP_COOLDOWN_MS = 260;

const STACK_LAYER_PRESETS = [
  { y: "0%", scale: 1, rotate: 0, zIndex: 20 },
  { y: "-5%", scale: 0.88, rotate: -1, zIndex: 10 },
  { y: "-8%", scale: 0.76, rotate: 1, zIndex: 5 },
  { y: "-10%", scale: 0.66, rotate: -0.5, zIndex: 0 },
] as const;
...
function CardStackCard({
  ...
  const { depth, isAnimating, pressActive, reducedMotion, throwImpulse, layers } = useCardStack();
  const total = stackDepth ?? depth;
  const effectiveLayer = layer ?? layers[0]!;
  const effectiveIndex = stackIndex ?? 0;

  const initial = getCardStackInitial(effectiveIndex, depth, effectiveLayer);
  const exit = getCardStackExit(effectiveIndex, effectiveLayer, reducedMotion, throwImpulse);
  const baseScale = getLayerScale(effectiveLayer);
  const isPressed = effectiveIndex === 0 && pressActive && !reducedMotion && !isAnimating;
  const animateTarget =
    typeof effectiveLayer.animate === "object" && effectiveLayer.animate !== null && !Array.isArray(effectiveLayer.animate)
      ? effectiveLayer.animate
      : {};
  const animate = isPressed
    ? { ...animateTarget, scale: baseScale * PRESS_SCALE_FACTOR }
    : effectiveLayer.animate;
  const transition = isPressed ? PRESS_SPRING : effectiveLayer.transition;

  return (
    <motion.div
      className={cn(
        "absolute inset-x-0 top-0 w-full will-change-transform motion-reduce:transition-none",
        effectiveLayer.className,
        className,
      )}
      aria-roledescription={`Card ${effectiveIndex + 1} of ${total}`}
      style={{
        transformOrigin: CARD_STACK_STACK_ORIGIN,
        ...effectiveLayer.style,
        ...style,
      }}
      initial={initial}
      animate={animate}
